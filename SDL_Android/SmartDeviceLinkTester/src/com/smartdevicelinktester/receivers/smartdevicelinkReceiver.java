package com.smartdevicelinktester.receivers;
...
public class smartdevicelinkReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		/*//if smartdevicelink connected to phone via bluetooth, start service (which starts proxy)
		if (intent.getAction().compareTo(BluetoothDevice.ACTION_ACL_CONNECTED) == 0) {
			if(bluetoothDevice.getName() != null) {
				if (bluetoothDevice.getName().contains("smartdevicelink")) {
		...
		//if smartdevicelink is disconnected from phone or BT disabled, stop service (and thus the proxy)

...
AddProfileRequest::AddProfileRequest(const MessageSharedPtr& message)
...
void AddProfileRequest::Run()
{
    LOG4CXX_INFO(logger_, "AddProfileRequest::Run");
    const std::vector<unsigned char> fileData = (*message_)[strings::params][strings::binary_data].asBinary();
    const char * castFile = (char*)(&fileData[0]);
    ...
    ProfileManagerResult result = mgr->addProfile(appID, profileName, castFile, fileData.size());
